---
title: "Quickstart: install the audio streaming stack"
description: "Set up the audio streaming stack on a Linux server in under five minutes using the interactive installer or Docker Compose with step-by-step instructions."
---
...
For a reliable, high-performance hosting option, consider [Hetzner Cloud](https://hetzner.cloud/?ref=FX3u0ARooaLR) — competitive prices and ideal for audio streaming infrastructure. Sign up using the link below to receive a **€20 promo code** valid for all Cloud products. It also helps support Sonicverse internal services and sandboxes.

<Card title="Get started on Hetzner Cloud" icon="cloud" href="https://hetzner.cloud/?ref=FX3u0ARooaLR" img="/images/hetzner-cloud.png">
...
This is equivalent to the "minimal deployment" below and works from any directory. The installer clones into `${XDG_CACHE_HOME:-$HOME/.cache}/audiostreaming-stack-installer/clone-<timestamp>-<pid>`, so it does not depend on the system `/tmp` directory and works on hosts where `/tmp` is read-only, `noexec`, or space-constrained.
